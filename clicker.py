...
def shop(db):
    for item in db:
        ...
        #load URL in Chrome
        load_url(itemURL, failed)

        #add to cart
        click_add(itemURL, quantity, failed)
            
        #update quantity
        click_plus(quantity, itemURL, failed)
...
#db = load_workbook("Steward Logistics.xlsx")
